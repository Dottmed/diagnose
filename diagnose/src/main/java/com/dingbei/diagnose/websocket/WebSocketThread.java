...
public class WebSocketThread extends Thread {
    ...
    public void run() {
        ...
        //为了防止WebSocketServiceConnectManager中绑定WebSocketService后，ServiceConnection绑定成功回调中
        //WebSocketService还没执行addListener（），延时启动WebSocket的连接
        mHandler.sendEmptyMessageDelayed(MessageType.CONNECT, 500);
        Looper.loop();
    }
    ...
    private class WebSocketHandler extends Handler {
        ...
        private boolean mIsRemoteClose;
                    ...
                    if (mWebSocket == null) {
                        ...
                        mWebSocket = new WebSocketClient(new URI(connectUrl), new Draft_6455()) {
                            ...
                            public void onMessage(String message) {
                                connectStatus = 2;
                                AppLogger.d("WebSocket 接收到消息：" + message);
                            ...
                            public void onClose(int code, String reason, boolean remote) {
                                mIsRemoteClose = remote;
                                connectStatus = 0;
                                AppLogger.d(remote ? "服务器关闭连接" : "WebSocket 断开连接");
                                if (mSocketListener != null) {
                            ...
                            public void onError(Exception ex) {
                                AppLogger.e("WebSocketClient#onError(Exception)", ex);
                            }
                        };
                        AppLogger.d("WebSocket 开始连接...");
                        mWebSocket.connect();
                    } else {
                        if(!mIsRemoteClose) { //服务器关闭连接不重连
                            AppLogger.d("WebSocket 开始重新连接...");
                            mWebSocket.reconnect();
                        }
                    }
                } catch (URISyntaxException e) {
                    connectStatus = 0;
                    AppLogger.d("WebSocket 连接失败");
                    if (mSocketListener != null) {
        ...
        private void disconnect() {
            if (connectStatus == 2) {
                AppLogger.d("正在关闭WebSocket连接");
                if (mWebSocket != null) {
                    mWebSocket.close();
                }
                connectStatus = 0;
                AppLogger.d("WebSocket连接已关闭");
            }
        }
        ...
        private void send(String text) {
            if (mWebSocket != null && connectStatus == 2) {
                try {
                    ...
                    AppLogger.d("数据发送成功：" + text);
                } catch (WebsocketNotConnectedException e) {
                    connectStatus = 0;
                    AppLogger.e("send()" + text, e);
                    AppLogger.e("连接已断开，数据发送失败：" + text, e);
                    if (mSocketListener != null) {
                        mSocketListener.onDisconnected();
