...
public class XhsEmoticonsKeyBoard extends AutoHeightLayout implements View.OnClickListener, EmoticonsFuncView.OnEmoticonsPageViewListener,
    ...
    protected void inflateKeyboardBar() {
        mInflater.inflate(R.layout.dingbei_view_keyboard_xhs_input, this);
    }

    protected View inflateFunc() {
        return mInflater.inflate(R.layout.dingbei_view_func_emoticon, null);
    }
...
//        mEtChat.setOnBackKeyClickListener(this);
        ...
        mBtnVoiceOrText.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mBtnVoiceOrText.setFocusable(mBtnVoiceOrText.getVisibility() == View.VISIBLE);
        });

        mBtnVoice.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mBtnVoice.setFocusable(mBtnVoice.getVisibility() == View.VISIBLE);
        });

        mBtnFace.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mBtnFace.setFocusable(mBtnFace.getVisibility() == View.VISIBLE);
        });

        mBtnMultimedia.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mBtnMultimedia.setFocusable(mBtnMultimedia.getVisibility() == View.VISIBLE);
        });

        mBtnSend.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mBtnSend.setFocusable(mBtnSend.getVisibility() == View.VISIBLE);
        });

        mEtChat.getViewTreeObserver().addOnGlobalLayoutListener(() -> {
            mEtChat.setFocusable(mEtChat.getVisibility() == View.VISIBLE);
        });
    ...
    public ImageView getBtnMultimedia() { return mBtnMultimedia; }
